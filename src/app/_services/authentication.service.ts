...
export class AuthenticationService {
    ...
    login(username: string, password: string) {
        ...
        return this.http.post<any>(`${environment.protocol + environment.urlCloud + environment.token}`, { "email": username, "senha": password }, this.httpOptions)
            .pipe(map(user => {
    ...
    alterarSenha(id: number, password: Password){
        return this.http.post<any>(`${environment.protocol + environment.urlCloud}/token/alterarSenha?id=${id}`
    ...
    acessosModulos(id: string, acesso: boolean) : Observable<AcessoModulo[]> {
      ...
      if(id.toString() != 'undefined'){
        return this.http.get<AcessoModulo[]>(`${environment.protocol + environment.urlCloud}/access/acessoModulo/filtro?idUsuario=${id}&acesso=${acesso}&content=true&size=100000&sort=nomeFuncionalidade&page=0&direction=DESC`)
      }
    }
    ...
    acessosFuncionalidades(id: string, idModulo: string, acesso: boolean) : Observable<AcessoFuncionalidade[]> {
      ...
      if(id.toString() != 'undefined' || idModulo.toString() != 'undefined'){
        return this.http.get<AcessoFuncionalidade[]>(`${environment.protocol + environment.urlCloud}/access/acessoFuncionalidade/filtro?id=${id}&idModulo=${idModulo}&acesso=${acesso}&pag=0&ord=id&dir=ASC&size=1000000`)
      }
    }
